...
public class Login implements Initializable {
    ...
    @FXML private TextField username;//关联用户输入的用户名
    @FXML private TextField password;//关联用户输入的密码
    ...
    private void enter() {
        ...
        Admin admin = adminService.get(username.getText());//根据用户名将数据查出来返回
        ...
        if(admin.getUserName().equals(this.username.getText()) && admin.getPassword().equals(this.password.getText())){
            ...
            App.decorator.setContent(ViewManager.getInstance().get("main"));//跳转到登录成功之后的界面
